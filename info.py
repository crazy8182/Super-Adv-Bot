...
# ============================
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '6859451629').split()] # Replace with the actual admin ID(s) to add
...
AUTH_CHANNEL = [int(fch) if id_pattern.search(fch) else fch for fch in environ.get('AUTH_CHANNEL', '-1002429092558').split()]
MULTI_FSUB = [int(channel_id) for channel_id in environ.get('MULTI_FSUB', '-1002429092558').split() if re.match(r'^-?\d+$', channel_id)]  # Channel for force sub (make sure bot is admin)
